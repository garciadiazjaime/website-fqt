...
def update_ecocapsulas(request):
	...
	for playlist_video_entry in playlist_video_feed.entry:
		...
		if len(flag) == 0:
			...
			cat = Categoria(name=name, url=url, slug=slugify(name))
			cat.save()
			category_id = cat.id
			c += 1
		else:
		...
		for row in playlist_channel_feed.entry:
			...
			url_data = urlparse.urlparse(source)
			query = urlparse.parse_qs(url_data.query)
			video_id = query["v"][0]

			flag2 = Ecocapsulas.objects.filter(source=source)
			if len(flag2) == 0:
				eco = Ecocapsulas(title=title, source=source, category_id=category_id, video_id=video_id)
				eco.save()
				v += 1
